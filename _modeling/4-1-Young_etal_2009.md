---
title: "Young etal 2009"
permalink: /modeling/young-2009/
---
A simplified weatherability-driven climate model.

**Reference**: Young, S.A., Saltzman, M.R., Foland, K.A., Linder, J.S., and Kump, L.R., 2009, A major drop in seawater 87Sr/86Sr during the Middle Ordovician (Darriwilian): Links to volcanism and climate? Geology, v. 37, p. 951–954, doi:10.1130/G30152A.1.

[link to paper](https://doi.org/10.1130/G30152A.1)
...
Phosphate in the ocean (\\(M_{OP}\\))

$$\frac{d(M_{OP})}{dt}=F_{wp} - F_{bp}$$

where:

\\(M_{OP}\\) = the amount of phosphate in the oceans, initial value = \\(3 \times 10^3(\times 10^{12}mol/ky)\\)
...
![png](/assets/images/blog/2026-07-05-Young_etal_2009-Young_etal_2009_14_1.png)
...
![png](/assets/images/blog/2026-07-05-Young_etal_2009-Young_etal_2009_17_1.png)
...
![png](/assets/images/blog/2026-07-05-Young_etal_2009-Young_etal_2009_19_1.png)
...
ODEs are solved numerically using the forward euler method.
...
for i in range(t-1):
    ...
    moc[i+1] = moc[i] + dt*Moc(fworg[i+1], fvolc[i+1], fborg[i+1], fwsil[i+1])
    ...
    mop[i+1] = mop[i] + dt*Mop(fwp[i+1],fbp[i+1])
    
        
    rr[i+1] = Rr(time[i], W[i])
    fwsr[i+1] = Fwsr(W[i], cwf[i])
    ro[i+1] = ro[i] + dt*Ro(ro[i], rb[i+1], rr[i+1], fwsr[i+1], fvolc[i+1], fhyd[i+1], mosr[i+1])
```
...
![png](/assets/images/blog/2026-07-05-Young_etal_2009-Young_etal_2009_26_0.png)
